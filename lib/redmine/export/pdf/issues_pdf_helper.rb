    ...
    module PDF
      module IssuesPdfHelper
        ...
        def issue_to_pdf(issue, assoc={})
          ...
          right << [l(:label_spent_time), "#{l_hours(issue.total_spent_hours)} (#{issue.done_ratio} #{l(:field_done_ratio)})"] if User.current.allowed_to?(:view_time_entries, issue.project)
